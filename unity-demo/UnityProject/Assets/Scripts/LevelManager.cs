...
public class LevelManager : MonoBehaviour
{
    ...
    [SerializeField]
    bool _showDSRequestPrompt = false;
    float _nextFlash;
    ...
    protected virtual bool SuppressLegacySceneUi
    {
        get { return false; }
    }

    protected virtual bool SuppressLegacyTutorialUi
    {
        get { return false; }
    }
    ...
    {
        _input = _player != null ? _player.GetComponent<InputManagement>() : null;
        if (SuppressLegacyTutorialUi)
        {
            SetLegacyTutorialUiActive(false);
        }
        else if(GameManager.Instance != null && GameManager.Instance._isLeftHanded)
        {
            SetActiveList(_tutorial, false);
            SetActiveList(_altTutorial, true);
        }
        ApplyLegacyUiSuppression();
        OnDSValueChanged.AddListener(ManageDS);
    }

    protected void ApplyLegacyUiSuppression()
    {
        if (SuppressLegacySceneUi)
            SetLegacySceneUiActive(false);
        if (SuppressLegacyTutorialUi)
            SetLegacyTutorialUiActive(false);
    }

    protected void SetLegacySceneUiActive(bool active)
    {
        if (_sceneUI == null)
            return;

        _sceneUI.enabled = active;
        _sceneUI.gameObject.SetActive(active);
    }

    protected void SetLegacyTutorialUiActive(bool active)
    {
        SetActiveList(_tutorial, active);
        SetActiveList(_altTutorial, active);
    }

    static void SetActiveList(List<GameObject> objects, bool active)
    {
        if (objects == null)
            return;
    ...
    internal virtual void Update()
    {
        ...
        if (!SuppressLegacySceneUi && _sceneUI != null && Keyboard.current[Key.LeftCtrl].isPressed && Keyboard.current[Key.H].wasPressedThisFrame)
        ...
        if (_started && !_ended)
        {
            ...
            {
                if (_dsValue != null && _discomfortScale < 10)
                {
            ...
            {
                if (_dsValue != null && _discomfortScale > 0)
                {
                    _dsValue.text = (_discomfortScale - 1).ToString();
                }
            }
        ...
        if(_showDSRequestPrompt && _dsArrow != null && Time.time >= _nextDSRequest)
        {
            _flashArrow = true;
        ...
        if (_showDSRequestPrompt && _dsArrow != null && _flashArrow)
        {
    ...
    public virtual void StartGame()
    {
        if (!_started)
        {
            ...
            _nextDSRequest = _showDSRequestPrompt ? Time.time + 60 : float.MaxValue;
            /*if (Application.isEditor)
                _nextDebugResult = Time.time + _debugResultTime;*/
    ...
    public virtual void EndGame(float loadAfter = 0)
    {
        if (_started && !_ended)
        {
            ...
            string result = GetResultString(GetResults(_complTime), sep);
            if (GameManager.Instance != null)
            {
                GameManager.Instance.ResultToFile(result);
        ...
        if (GameManager.Instance!=null)
            switch (GameManager.Instance._technique)
            {
                ...
                case MitigationTechnique.None:
                    break;
                case MitigationTechnique.GazeContingentDOF:
                    Debug.LogWarning("GazeContingentDOF is disabled in the Roller-only project because the Standard Assets image effect is not included.");
                    break;
                case MitigationTechnique.VRTunnellingPro:
                    ...
                    break;
                default:
                    if (techniqueMap.TryGetValue(GameManager.Instance._technique, out Type type))
                    {
                        var technique = PlayerHead.GetComponent(type) as MonoBehaviour;
                        if (technique != null)
